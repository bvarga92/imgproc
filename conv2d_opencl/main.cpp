...
/* Idomeres eredmenye: 229.8 Mpixel/s                                                                          */
/***************************************************************************************************************/
...
#define KERNELFILE  "_src/kernel_16x16.cl"
#define TB_X 16
#define TB_Y 16
...
void convOpenCL(int imgHeight, int imgWidth, unsigned char *imgSrc, unsigned char *imgDst){
	...
	device_coeffs=clCreateBuffer(context,CL_MEM_READ_ONLY,FILTER_W*FILTER_H*sizeof(float),NULL,NULL);
	queue=clCreateCommandQueue(context,device,CL_QUEUE_PROFILING_ENABLE,NULL);
	clEnqueueWriteBuffer(queue,device_imgSrc,CL_TRUE,0,imgHeightF*imgWidthF*3,imgSrc,0,NULL,NULL);
	clEnqueueWriteBuffer(queue,device_coeffs,CL_TRUE,0,FILTER_W*FILTER_H*sizeof(float),filter_laplace,0,NULL,NULL);
	clSetKernelArg(kernel,0,sizeof(device_imgSrc),&device_imgSrc);
	clSetKernelArg(kernel,1,sizeof(device_imgDst),&device_imgDst);
	clSetKernelArg(kernel,2,sizeof(device_coeffs),&device_coeffs);
	clSetKernelArg(kernel,3,sizeof(int),&imgWidthF);
	clFinish(queue);
	/* kernel futtatasa */
...
int main(){
	...
	convOpenCL(imgHeight, imgWidth, imgSrc, imgRes);
	/* eredmeny kiirasa */
	for(int row=0; row<imgHeight; row++)
		for(int col=0; col<imgWidth; col++){
