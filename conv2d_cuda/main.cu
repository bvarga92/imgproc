...
/* Idomeres eredmenye: 266.6 Mpixel/s                                                                          */
/***************************************************************************************************************/
...
#define OUTPUTFILE  "_images/img1_laplace.jpg"
#define KERN_32X8 1

const float filter_laplace[25]={-1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
                                -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
                                -1.0f, -1.0f, 24.0f, -1.0f, -1.0f,
                                -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
                                -1.0f, -1.0f, -1.0f, -1.0f, -1.0f};

extern __global__ void conv_kernel_16x16(unsigned char* gInput, unsigned char* gOutput, float *coeffs, int imgWidthF);
extern __global__ void conv_kernel_32x8(unsigned char* gInput, unsigned char* gOutput, float *coeffs, int imgWidthF);
...
void convCUDA(int imgHeight, int imgWidth, unsigned char *imgSrc, unsigned char *imgDst){
	...
	cudaMalloc((void**)&device_coeffs,FILTER_W*FILTER_H*sizeof(float));
	cudaMemcpy(device_imgSrc,imgSrc,imgWidthF*imgHeightF*3,cudaMemcpyHostToDevice); 
	cudaMemcpy(device_coeffs,filter_laplace,FILTER_W*FILTER_H*sizeof(float),cudaMemcpyHostToDevice);
	...
	s0=clock();
		...
		for(int i=0;i<RUNS;i++) conv_kernel_32x8<<<thrGrid,thrBlock>>>(device_imgSrc,device_imgDst,device_coeffs,imgWidthF);
	#else
		dim3 thrBlock(16,16);
		dim3 thrGrid(imgWidth/16,imgHeight/16);
		for(int i=0;i<RUNS;i++) conv_kernel_16x16<<<thrGrid,thrBlock>>>(device_imgSrc,device_imgDst,device_coeffs,imgWidthF);
	#endif
