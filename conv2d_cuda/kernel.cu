...
__global__ void conv_kernel_16x16(unsigned char* gInput, unsigned char* gOutput, float *coeffs, int imgWidthF){
	...
	for(int fy=0;fy<5;fy++){
		...
		for(int fx=0;fx<5;fx++){
			float coeff=coeffs[fy*5+fx];
			#pragma unroll
			for(int rgb=0;rgb<3;rgb++) acc[rgb]=fmaf(coeff,in_shmem[threadIdx.y+fy][threadIdx.x+fx][rgb],acc[rgb]);
		}
	}
	/* 0...255 tartomanyba csonkolas es kiiras */
	#pragma unroll
	for(int rgb=0;rgb<3;rgb++){
		acc[rgb]=fminf(acc[rgb],255.0f);
		acc[rgb]=fmaxf(acc[rgb],0.0f);
		gOutput[out_pix+rgb]=(unsigned char)(acc[rgb]);
	}
}

__global__ void conv_kernel_32x8(unsigned char* gInput, unsigned char* gOutput, float *coeffs, int imgWidthF){
	int row     = blockIdx.y*blockDim.y+threadIdx.y+2;
	int col     = blockIdx.x*blockDim.x+threadIdx.x+2;
	int thid_1d = threadIdx.y*blockDim.x+threadIdx.x;
	int ld_rgb  = thid_1d %3;
