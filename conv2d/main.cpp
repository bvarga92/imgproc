...
/* Idomeres eredmenyei (release modban!):                                                                      */
/*    - sima: 19.4 Mpixel/s                                                                                    */
/*    - OMP: 34.2 Mpixel/s                                                                                     */
/*    - SSE: 27.1 Mpixel/s                                                                                     */
/*    - SSE+OMP: 61.6 Mpixel/s                                                                                 */
/***************************************************************************************************************/
...
void (*conv)(int,int,const float*,const float*,float*)=&conv_filter_sse_omp; //melyik implementaciot hasznaljuk
const float *filter=filter_laplace; //melyik szurot futtassuk
#define INPUTFILE   "img1.bmp" //bemeneti fajlnev
#define OUTPUTFILE  "img1_laplace.bmp" //kimeneti fajlnev
...
int main(){
	...
	s0=clock();
	for(int r=0; r<RUNS; r++) conv(imgHeight, imgWidth, filter, imgFloat, imgFloatRes);
	e0=clock();
